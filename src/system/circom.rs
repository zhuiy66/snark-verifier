use crate::{
    util::{
        arithmetic::{
            Curve, CurveAffine, CurveExt, Domain, Field, GroupEncoding, MultiMillerLoop,
...
pub fn compile<M: MultiMillerLoop>(vk: &VerifyingKey<M>) -> Protocol<M::G1Affine> {
    ...
    let quotient = {
        ...
        let numerator = Expression::DistributePowers(
...
#[derive(Clone, Debug)]
pub struct PublicSignals<F: PrimeField>(pub Vec<F>);
